...
void kruskal(int n,priority_queue<tuple<int,int,int>,vector<tuple<int,int,int>>>& edges,vector<int>& parent,vector<tuple<int,int,int>>& mst)
{
    vector<int> rank(n,0);
    while(!edges.empty())
    {
...
int main()
{
    ...
    priority_queue<tuple<int,int,int>,vector<tuple<int,int,int>>> edges;
    for(int i=0;i<n;i++)
    {
        for(int j=i+1;j<n;j++)
        {
    ...
    cout<<"Maximum Spanning Weight : "<<minw<<endl;
    return 0;
}